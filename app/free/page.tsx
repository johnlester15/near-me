...
function Reveal({ children, delay = 0, direction = "up" }: {
  ...
  const hidden = { up: "opacity-0 translate-y-8", left: "opacity-0 -translate-x-8", right: "opacity-0 translate-x-8", fade: "opacity-0" }[direction];
  return (
...
type ChatMessage = { id: number; sender: "customer" | "professional"; text: string; time: string; };
type Inquiry = {
  id: number; customer: string; email: string; phone: string; initialMessage: string;
  time: string; read: boolean; avatar: string; service: string; location: string; messages: ChatMessage[];
};
type Job = {
  id: number; customer: string; email: string; phone: string; service: string;
  location: string; date: string; status: JobStatus; avatar: string;
  description: string; address: string; preferredSchedule: string; notes: string;
};

// ─── Plan config ──────────────────────────────────────────────────────────────
const PLAN_CONFIG: Record<Plan, {
  label: string; price: string; gradient: string; bg: string; border: string;
  textColor: string; badgeBg: string; Icon: React.ElementType;
  features: string[]; limits: { inquiries: string; visibility: string; position: string; video: boolean; featured: boolean; analytics: boolean };
}> = {
  free: {
    label: "Free", price: "₱0/mo",
    gradient: "from-gray-500 to-gray-400", bg: "bg-gray-50", border: "border-gray-200",
    textColor: "text-gray-600", badgeBg: "bg-gray-100", Icon: User,
    features: ["Visible to customers (limited)", "3 inquiries per month", "Listed at bottom of results"],
    limits: { inquiries: "3/month", visibility: "Bottom listing", position: "Last", video: false, featured: false, analytics: false },
  },
  basic: {
    label: "Basic", price: "₱199/mo",
    gradient: "from-blue-500 to-blue-400", bg: "bg-blue-50", border: "border-blue-200",
    textColor: "text-blue-600", badgeBg: "bg-blue-100", Icon: BadgeCheck,
    features: ["Unlimited inquiries", "Full visibility", "Verified badge", "Listed in all results"],
    limits: { inquiries: "Unlimited", visibility: "Full listing", position: "Standard", video: false, featured: false, analytics: false },
  },
  pro: {
    label: "Pro", price: "₱399/mo",
    gradient: "from-purple-500 to-purple-400", bg: "bg-purple-50", border: "border-purple-200",
    textColor: "text-purple-600", badgeBg: "bg-purple-100", Icon: Zap,
    features: ["Featured section placement", "Priority in search results", "Analytics dashboard", "Unlimited inquiries"],
    limits: { inquiries: "Unlimited", visibility: "Featured section", position: "Priority", video: false, featured: true, analytics: true },
  },
  premium: {
    label: "Premium", price: "₱699/mo",
    gradient: "from-amber-500 to-amber-400", bg: "bg-amber-50", border: "border-amber-200",
    textColor: "text-amber-600", badgeBg: "bg-amber-100", Icon: Crown,
    features: ["Sponsored top placement", "Introduction video on profile", "Maximum exposure", "Analytics + priority support"],
    limits: { inquiries: "Unlimited", visibility: "Sponsored top", position: "Top", video: true, featured: true, analytics: true },
  },
};

// ─── Mock data ────────────────────────────────────────────────────────────────
// ⬇ Change this to test different plans: "free" | "basic" | "pro" | "premium"
const DEMO_PLAN: Plan = "free";
const VIDEO_URL = "https://www.youtube.com/embed/DTzHOdfACPM";

const INITIAL_INQUIRIES: Inquiry[] = [
  {
    id: 1, customer: "Grace Flores", email: "grace@gmail.com", phone: "+63 955 123 4567",
    initialMessage: "Hi! I need a plumber for a burst pipe in our kitchen. Can you come this weekend?",
    time: "10 min ago", read: false, avatar: "GF", service: "Burst Pipe", location: "Davao City",
    messages: [{ id: 1, sender: "customer", text: "Hi! I need a plumber for a burst pipe in our kitchen. Can you come this weekend?", time: "10:02 AM" }],
  },
  {
    id: 2, customer: "Ben Tan", email: "ben@gmail.com", phone: "+63 966 234 5678",
    initialMessage: "Good day! Looking for someone to install a new bathroom set. How much do you charge?",
    time: "1 hr ago", read: false, avatar: "BT", service: "Bathroom Installation", location: "Tagum City",
    messages: [{ id: 1, sender: "customer", text: "Good day! Looking for someone to install a new bathroom set. How much do you charge?", time: "9:15 AM" }],
  },
  {
    id: 3, customer: "Joy Cruz", email: "joy@gmail.com", phone: "+63 977 345 6789",
    initialMessage: "Hello, I need help with a leaking pipe under the sink. Are you available Monday morning?",
    time: "Yesterday", read: true, avatar: "JC", service: "Pipe Leak", location: "Digos City",
    messages: [
      { id: 1, sender: "customer", text: "Hello, I need help with a leaking pipe under the sink. Are you available Monday morning?", time: "Yesterday 2:30 PM" },
  ...
  {
    id: 4, customer: "Noel Santos", email: "noel@gmail.com", phone: "+63 988 456 7890",
    initialMessage: "Need full bathroom renovation plumbing. Please send quotation.",
    time: "2 days ago", read: true, avatar: "NS", service: "Bathroom Renovation", location: "Davao City",
    messages: [
      { id: 1, sender: "customer", text: "Need full bathroom renovation plumbing. Please send quotation.", time: "2 days ago 11:00 AM" },
      { id: 2, sender: "professional", text: "Hello Noel! My rate starts at ₱8,000. Can we schedule a site visit?", time: "2 days ago 11:45 AM" },
    ],
  },
];

const INITIAL_JOBS: Job[] = [
  { id: 1, customer: "Maria Santos", email: "maria@gmail.com", phone: "+63 912 111 2222", service: "Pipe Repair", location: "Davao City", address: "123 Quezon Blvd., Brgy. Talomo, Davao City", date: "Mar 20, 2025", status: "awaiting_confirm", avatar: "MS", description: "Burst pipe in the kitchen causing water to flood the floor.", preferredSchedule: "Anytime this week, preferably morning", notes: "Gate code is 1234. Please call before arriving." },
  { id: 2, customer: "Carlo Reyes", email: "carlo@gmail.com", phone: "+63 922 333 4444", service: "Water Heater Install", location: "Tagum City", address: "45 Rizal St., Brgy. Visayan Village, Tagum City", date: "Mar 18, 2025", status: "in_progress", avatar: "CR", description: "Need a new electric water heater installed. Customer already purchased an Ariston 30L unit.", preferredSchedule: "Saturday morning, March 22", notes: "Second floor bathroom. Bring own tools." },
  { id: 3, customer: "Ana Lim", email: "ana@gmail.com", phone: "+63 933 555 6666", service: "Drainage Fix", location: "Davao City", address: "78 Pichon St., Brgy. Agdao, Davao City", date: "Mar 15, 2025", status: "completed", avatar: "AL", description: "Slow drainage in bathroom and kitchen.", preferredSchedule: "March 15, afternoon", notes: "Job completed. Customer confirmed." },
  { id: 4, customer: "Rodel Manalo", email: "rodel@gmail.com", phone: "+63 944 777 8888", service: "Faucet Replacement", location: "Panabo City", address: "12 National Highway, Brgy. New Visayas, Panabo City", date: "Mar 10, 2025", status: "completed", avatar: "RM", description: "Kitchen faucet leaking at base and handle.", preferredSchedule: "March 10, any time", notes: "Completed. Water shutoff beside kitchen cabinet." },
];
...
function InfoRow({ Icon, label, value }: { Icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-gray-50 last:border-0">
      <div className="w-7 h-7 rounded-lg bg-amber-50 flex items-center justify-center shrink-0 mt-0.5"><Icon size={12} className="text-amber-600" /></div>
...
function UpgradeBanner({ currentPlan, targetPlan, message, compact = false }: {
  currentPlan: Plan; targetPlan: Plan; message: string; compact?: boolean;
}) {
  ...
  if (compact) {
    return (
      <div className={`flex items-center gap-3 p-3 rounded-xl ${cfg.bg} border ${cfg.border}`}>
        ...
        <button className={`shrink-0 h-7 px-3 rounded-lg text-[11px] font-bold text-white bg-gradient-to-r ${cfg.gradient} hover:opacity-90 transition-all`}>
          Upgrade
        </button>
      </div>
    );
    ...
    <div className={`rounded-2xl border ${cfg.border} overflow-hidden`}>
      <div className={`bg-gradient-to-r ${cfg.gradient} p-4 text-white relative overflow-hidden`}>
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-white/70 mb-0.5">Upgrade to {cfg.label}</p>
      ...
      <div className={`${cfg.bg} px-4 py-3`}>
        ...
        <button className={`w-full h-10 rounded-xl text-white text-sm font-bold bg-gradient-to-r ${cfg.gradient} hover:opacity-90 active:scale-[0.98] transition-all shadow-md flex items-center justify-center gap-2`}>
    ...
    <div className={`relative rounded-2xl overflow-hidden border ${cfg.border} shadow-sm`}>
      <div className={`bg-gradient-to-r ${cfg.gradient} p-4 text-white relative overflow-hidden`}>
        ...
        <div className="relative flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1"><Icon size={15} /><span className="text-[11px] font-bold uppercase tracking-widest text-white/70">Current Plan</span></div>
            <p className="text-xl font-extrabold">{cfg.label} — {cfg.price}</p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center border border-white/30"><CheckCheck size={18} className="text-white" /></div>
        </div>
      </div>
      <div className={`${cfg.bg} px-4 py-3`}>
      ...
      <div className="px-4 py-3 bg-white flex items-center justify-between border-t border-gray-100">
        <div className="flex items-center gap-1.5 text-xs text-gray-500"><Clock size={11} className="text-gray-400" /><span>Renews: <span className="font-bold text-gray-700">April 30, 2025</span></span></div>
        {plan !== "premium" && (
          <button className={`flex items-center gap-1.5 h-7 px-3 rounded-lg text-[11px] font-bold text-white bg-gradient-to-r ${PLAN_CONFIG["premium"].gradient} hover:opacity-90 transition-all`}>
            <Crown size={10} /> Upgrade
          </button>
        )}
        {plan === "premium" && <div className="flex items-center gap-1 text-[11px] font-bold text-amber-600"><Star size={10} className="fill-amber-400 text-amber-400" /> Best Plan</div>}
      </div>
    </div>
  );
}
...
// ─── VIDEO MANAGER ────────────────────────────────────────────────────────────
function VideoManager({ plan, videoUrl, onSave }: {
  plan: Plan; videoUrl?: string; onSave: (url: string) => void;
}) {
  ...
  function handleSave() {
    onSave(getEmbedUrl(input));
    setSaved(true);
    setTimeout(() => setSaved(false), 3000);
  }

  if (plan !== "premium") {
    const nextPlan = plan === "pro" ? "premium" : plan === "basic" ? "pro" : "basic";
    return (
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 border border-gray-200">
            ...
            <span className="text-[10px] font-bold text-gray-500">Premium Only</span>
          </div>
        </div>
        <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-5 flex flex-col items-center text-center gap-3 mb-3">
          <div className="w-12 h-12 rounded-2xl bg-gray-100 flex items-center justify-center"><Video size={22} className="text-gray-400" /></div>
          <div>
            <p className="text-sm font-bold text-gray-600 mb-0.5">Video Introduction</p>
            <p className="text-xs text-gray-400 leading-relaxed">Add a YouTube video to your profile so customers can see your work before hiring you.</p>
          </div>
          <div className="text-[11px] text-gray-400">Available for <span className="font-bold text-amber-600">Premium ₱699/mo</span> only</div>
        </div>
        <UpgradeBanner currentPlan={plan} targetPlan="premium" message="Add a video to attract more customers" compact />
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-amber-200 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        {saved && <div className="flex items-center gap-1 text-green-600"><CheckCircle size={13} /><span className="text-xs font-bold">Saved!</span></div>}
      </div>

      {videoUrl && (
        <div className="mb-4">
          <div className="relative w-full rounded-xl overflow-hidden bg-gray-900 shadow-md mb-2" style={{ paddingBottom: "56.25%" }}>
            {!playing ? (
              ...
              >
                ...
                <div className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-black/60 px-2.5 py-1 rounded-full">
                  <div className="w-3 h-3 bg-red-500 rounded-sm flex items-center justify-center shrink-0"><Play size={7} className="text-white fill-white ml-0.5" /></div>
                  <span className="text-[9px] font-bold text-white/80">YouTube</span>
                </div>
              </div>
            ) : (
              <iframe className="absolute inset-0 w-full h-full" src={`${videoUrl}?autoplay=1&rel=0&modestbranding=1&playsinline=1`} title="Your introduction video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
            )}
          </div>
          <p className="text-[10px] text-gray-400 text-center">Your video is live on your profile ✓</p>
        </div>
      )}
      ...
      <div className="space-y-3">
        <div className="flex items-start gap-2.5 p-3 rounded-xl bg-blue-50 border border-blue-200">
          ...
          <div className="text-xs text-blue-700 leading-relaxed">
            <p className="font-bold mb-0.5">How to add your video:</p>
            <p>1. Upload your video to YouTube (can be unlisted)</p>
            <p>2. Copy the video link and paste below</p>
            <p className="mt-1 text-blue-500 font-medium">Coming soon: Direct upload from your phone via Cloudinary</p>
          </div>
        </div>
        <div>
          <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">YouTube Video Link</label>
          <div className="relative">
            <Link size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <input type="url" value={input} onChange={e => { setInput(e.target.value); setPlaying(false); }} placeholder="https://www.youtube.com/watch?v=..."
        ...
        <div className="flex gap-2">
          <button onClick={handleSave} disabled={!input.trim()} className="flex-1 h-10 rounded-xl bg-gradient-to-r from-amber-600 to-amber-400 text-white text-sm font-bold hover:opacity-90 active:scale-[0.98] transition-all shadow-md shadow-amber-200 flex items-center justify-center gap-2 disabled:opacity-40">
            <Save size={13} /> Save Video
          </button>
          {input && (
            <button onClick={() => { setPreview(!preview); setPlaying(false); }} className="h-10 px-4 rounded-xl border border-gray-200 text-gray-600 text-sm font-bold hover:border-amber-300 transition-all flex items-center gap-1.5">
              <Eye size={13} /> Preview
            </button>
          )}
        </div>
        {preview && input && (
          <div className="relative w-full rounded-xl overflow-hidden bg-gray-900 shadow-md" style={{ paddingBottom: "56.25%" }}>
            <iframe className="absolute inset-0 w-full h-full" src={`${getEmbedUrl(input)}?rel=0&modestbranding=1`} title="Video preview" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
          </div>
        )}
      </div>
    </div>
...
function ChatWindow({ inquiry, onBack, onSend }: {
  inquiry: Inquiry; onBack: () => void; onSend: (id: number, text: string) => void;
}) {
  ...
  function handleSend() {
    ...
    onSend(inquiry.id, t); setInput("");
  }

  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100 shrink-0">
        <button onClick={onBack} className="w-8 h-8 rounded-xl bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"><ArrowLeft size={15} className="text-gray-600" /></button>
        ...
        <button onClick={() => setShowInfo(!showInfo)} className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${showInfo ? "bg-amber-100" : "bg-gray-100 hover:bg-gray-200"}`}>
      ...
      {showInfo && (
        <div className="bg-amber-50 border-b border-amber-100 px-4 py-3 space-y-1.5 shrink-0">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-700"><Mail size={11} className="text-amber-500 shrink-0" /><span>{inquiry.email}</span></div>
          <div className="flex items-center gap-2 text-xs text-gray-700"><Phone size={11} className="text-amber-500 shrink-0" /><span>{inquiry.phone}</span></div>
          <div className="flex gap-2 mt-2">
            <a href={`mailto:${inquiry.email}`} className="flex-1 h-7 rounded-lg bg-amber-500 text-white text-[11px] font-bold flex items-center justify-center gap-1 hover:opacity-90"><Mail size={10} /> Email</a>
            <a href={`tel:${inquiry.phone}`} className="flex-1 h-7 rounded-lg border border-amber-300 text-amber-600 text-[11px] font-bold flex items-center justify-center gap-1 hover:bg-amber-100"><Phone size={10} /> Call</a>
          </div>
        </div>
      )}
        ...
        {inquiry.messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold shrink-0 mt-1 ${isMe ? "bg-gradient-to-br from-gray-700 to-gray-600" : "bg-gradient-to-br from-amber-500 to-amber-400"}`}>
                {isMe ? "JD" : inquiry.avatar}
              </div>
              <div className={`max-w-[75%] flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                <div className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-gradient-to-br from-amber-500 to-amber-400 text-white rounded-tr-sm" : "bg-white border border-gray-100 text-gray-800 shadow-sm rounded-tl-sm"}`}>
                  {msg.text}
                </div>
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1 bg-gray-50 rounded-2xl border border-gray-200 focus-within:border-amber-400 focus-within:bg-white transition-all px-4 py-2.5 min-h-[44px] flex items-center">
            <textarea rows={1} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
              placeholder="Type your reply..." className="flex-1 text-sm text-gray-800 placeholder-gray-400 bg-transparent focus:outline-none resize-none" style={{ maxHeight: 100 }}
            />
          </div>
          <button onClick={handleSend} disabled={!input.trim()} className="w-11 h-11 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-400 flex items-center justify-center shadow-md hover:opacity-90 active:scale-95 transition-all disabled:opacity-40 shrink-0">
...
function InboxTab({ plan }: { plan: Plan }) {
  ...
  // Free plan: only show first 3 inquiries (limit)
  ...
  function handleSend(id: number, text: string) {
    ...
    setInquiries(prev => prev.map(inq => inq.id !== id ? inq : { ...inq, messages: [...inq.messages, { id: inq.messages.length + 1, sender: "professional", text, time: timeStr }] }));
  }
        ...
        <div className="hidden md:flex flex-col w-72 shrink-0 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            {visibleInquiries.map(inq => (
              <button key={inq.id} onClick={() => openChat(inq)} className={`w-full text-left px-4 py-3 transition-all hover:bg-amber-50 ${activeId === inq.id ? "bg-amber-50 border-l-2 border-amber-500" : ""}`}>
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        <div className="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center"><Inbox size={17} className="text-amber-600" /></div>
        <div>
      ...
      {/* Free plan inquiry counter */}
      {plan === "free" && (
        <div className="mb-4 p-4 rounded-2xl bg-gray-50 border border-gray-200">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-bold text-gray-600">Monthly Inquiry Limit</p>
            <span className="text-xs font-bold text-gray-700">{usedCount} / {monthlyLimit} used</span>
          </div>
          <div className="h-2 w-full rounded-full bg-gray-200 mb-3">
            <div className="h-2 rounded-full bg-gradient-to-r from-amber-500 to-amber-400 transition-all" style={{ width: `${(usedCount / (monthlyLimit || 1)) * 100}%` }} />
          </div>
          <UpgradeBanner currentPlan="free" targetPlan="basic" message="Get unlimited inquiries — ₱199/mo" compact />
        </div>
      )}
      ...
      <div className="space-y-2.5">
        {visibleInquiries.map((inq, i) => {
          ...
          return (
            <Reveal key={inq.id} delay={i * 60} direction="up">
              <button onClick={() => openChat(inq)} className={`w-full text-left rounded-2xl border p-4 transition-all duration-200 hover:border-amber-300 hover:shadow-md hover:-translate-y-0.5 group ${inq.read ? "border-gray-100 bg-white" : "border-amber-200 bg-amber-50/50 shadow-sm"}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="flex items-center gap-1 text-[11px] text-gray-400"><MapPin size={9} />{inq.location}</span>
                    </div>
                    <p className="text-xs text-gray-400 line-clamp-1">{lastMsg.sender === "professional" ? "You: " : ""}{lastMsg.text}</p>
                  </div>
        ...
        {/* Locked inquiries for free plan */}
        {plan === "free" && inquiries.length > (monthlyLimit || 0) && (
          <div className="relative rounded-2xl border border-gray-200 bg-gray-50 p-4 overflow-hidden">
            <div className="absolute inset-0 bg-white/70 backdrop-blur-[2px] flex flex-col items-center justify-center z-10 gap-2">
              <Lock size={20} className="text-gray-400" />
              <p className="text-sm font-bold text-gray-600">Monthly limit reached</p>
              <p className="text-xs text-gray-400">Upgrade to see more inquiries</p>
            </div>
            <div className="opacity-20 pointer-events-none space-y-2">
              {inquiries.slice(monthlyLimit || 0).map(inq => (
                <div key={inq.id} className="flex items-center gap-3 py-2">
                  ...
                  <div className="flex-1"><div className="h-3 w-24 bg-gray-200 rounded mb-1" /><div className="h-2 w-32 bg-gray-200 rounded" /></div>
                </div>
              ))}
            </div>
          </div>
...
// ─── JOB DETAIL MODAL ─────────────────────────────────────────────────────────
function JobDetailModal({ job, onClose, onMarkDone }: { job: Job; onClose: () => void; onMarkDone: (id: number) => void }) {
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 max-h-[92vh] flex flex-col rounded-t-3xl bg-white overflow-hidden shadow-2xl md:inset-0 md:m-auto md:max-w-lg md:max-h-[88vh] md:rounded-3xl">
        ...
        <div className="px-5 pt-4 pb-3 flex items-center justify-between border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-2"><CheckSquare size={15} className="text-amber-500" /><p className="text-sm font-extrabold text-gray-900">Job Details</p></div>
          <div className="flex items-center gap-2">
            <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full border ${s.color} ${s.bg} ${s.border}`}>{s.label}</span>
            <button onClick={onClose} className="w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors"><X size={13} className="text-gray-500" /></button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4">
              <div className="flex items-start gap-2.5 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-orange-600 leading-relaxed">Notification sent to <span className="font-bold">{job.customer}</span>. Jobs Done updates after they confirm.</p>
                </div>
              </div>
              <div className="flex gap-2">
          ...
          {job.status === "awaiting_confirm" && (
            <div className="flex items-center gap-3 bg-orange-50 border border-orange-200 rounded-2xl p-4">
              ...
              <div><p className="text-sm font-bold text-orange-800">Waiting for Customer Confirmation</p><p className="text-xs text-orange-600 mt-0.5">Jobs Done updates once the customer confirms.</p></div>
            </div>
          )}
          {job.status === "completed" && (
            <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-2xl p-4">
              <CheckCircle size={16} className="text-green-500 shrink-0" />
              <div><p className="text-sm font-bold text-green-800">Job Completed & Confirmed</p><p className="text-xs text-green-600 mt-0.5">Counted in your Jobs Done total.</p></div>
            </div>
          )}
        </div>
      </div>
...
function DashboardTab({ jobs, plan }: { jobs: Job[]; plan: Plan }) {
  ...
  return (
    <div className="space-y-5">
      {/* FREE — big subscribe banner first */}
      {plan === "free" && (
        <Reveal direction="up">
          <div className="rounded-2xl overflow-hidden border-2 border-amber-300 shadow-lg">
            <div className="bg-gradient-to-r from-amber-600 via-amber-500 to-yellow-400 p-5 text-white relative overflow-hidden">
              ...
              <div className="relative">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-white/80">You are on Free Plan</span>
                </div>
                <p className="text-lg font-extrabold mb-1">Your profile is hidden from most customers!</p>
                <p className="text-sm text-white/80">Upgrade now to get full visibility and start getting more jobs.</p>
              </div>
            </div>
            <div className="bg-white p-4 space-y-2">
              {[
                ...
                const Icon = c.Icon;
                return (
                  <button key={p} className={`w-full flex items-center gap-3 p-3 rounded-xl ${c.bg} border ${c.border} hover:opacity-90 transition-all group`}>
                    <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${c.gradient} flex items-center justify-center shrink-0`}><Icon size={16} className="text-white" /></div>
                    <div className="flex-1 text-left">
                      <p className={`text-sm font-extrabold ${c.textColor}`}>{c.label} — {c.price}</p>
      ...
      <Reveal direction="up" delay={100}>
        <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-3">Your Visibility Status</p>
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className={`rounded-xl p-3 border text-center ${ok ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
                <p className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${ok ? "text-green-600" : "text-red-500"}`}>{label}</p>
                <p className={`text-xs font-extrabold ${ok ? "text-green-700" : "text-red-700"}`}>{val}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Analytics — pro/premium only */}
      {limits.analytics ? (
        <Reveal direction="up" delay={120}>
          <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-3">Analytics</p>
            <div className="grid grid-cols-3 gap-3">
              {[{ label: "Profile Views", val: "248" }, { label: "Inquiries", val: "12" }, { label: "Conversion", val: "4.8%" }].map(({ label, val }) => (
        ...
        <Reveal direction="up" delay={120}>
          <UpgradeBanner currentPlan={plan} targetPlan="pro" message="Unlock analytics to track your performance" />
        </Reveal>
      )}

      {/* Video status — premium only */}
      {plan === "premium" && (
        <Reveal direction="up" delay={140}>
          <div className="bg-white rounded-2xl border border-amber-200 p-4 shadow-sm flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500 to-red-400 flex items-center justify-center shadow-sm shrink-0"><Play size={20} className="text-white fill-white ml-0.5" /></div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-extrabold text-gray-900">Introduction Video</p>
              <p className="text-xs text-gray-400">Your video is live and visible to customers on your profile.</p>
            </div>
            <div className="flex items-center gap-1 shrink-0"><CheckCircle size={14} className="text-green-500" /><span className="text-xs font-bold text-green-600">Live</span></div>
          </div>
        </Reveal>
      )}
...
function JobsTab() {
  ...
  const counts = { all: jobs.length, in_progress: jobs.filter(j => j.status === "in_progress").length, awaiting_confirm: jobs.filter(j => j.status === "awaiting_confirm").length, completed: jobs.filter(j => j.status === "completed").length };
  return (
    <div>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {([{ key: "all", label: `All (${counts.all})` }, { key: "in_progress", label: `In Progress (${counts.in_progress})` }, { key: "awaiting_confirm", label: `Awaiting (${counts.awaiting_confirm})` }, { key: "completed", label: `Done (${counts.completed})` }] as { key: "all" | JobStatus; label: string }[]).map(({ key, label }) => (
    ...
    <div className="space-y-5 pb-10">
      ...
      <div className="pt-10 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs font-semibold text-amber-600">{form.category} · {form.type}</p>
        ...
        <button onClick={() => { if (editing) { setSaved(true); setTimeout(() => setSaved(false), 3000); } setEditing(!editing); }}
          className={`flex items-center gap-1.5 h-9 px-4 rounded-xl text-xs font-bold transition-all shrink-0 ${editing ? "bg-amber-500 text-white shadow-md" : "border border-gray-200 text-gray-600 hover:border-amber-300 bg-white"}`}
        >
          {editing ? <><Save size={13} /> Save</> : <><Edit3 size={13} /> Edit</>}
        </button>
      </div>

      {/* Profile visibility notice for free */}
      {plan === "free" && (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-3.5">
        ...
        <div>
          ...
          <textarea rows={3} value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} disabled={!editing}
            className={`w-full px-3 py-2.5 rounded-xl border text-sm text-gray-800 transition-all resize-none ${editing ? "border-amber-300 bg-white focus:outline-none focus:ring-2 focus:ring-amber-400/20" : "border-gray-100 bg-gray-50 text-gray-600 cursor-default"}`}
          />
        </div>
      </div>
      ...
      {/* Upgrade prompt at bottom for non-premium */}
      {plan !== "premium" && (
        <UpgradeBanner currentPlan={plan} targetPlan="premium" message="Unlock all Premium features for maximum exposure" />
      )}
    </div>
  );
}
...
export default function ProfessionalDashboard() {
  ...
  // ⬇ Change DEMO_PLAN at top to test: "free" | "basic" | "pro" | "premium"
        ...
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div className={`hidden sm:flex items-center gap-1 px-2 py-0.5 rounded-full ${cfg.badgeBg} border ${cfg.border} ml-1`}>
              ...
              <span className={`text-[10px] font-bold ${cfg.textColor} uppercase tracking-wider`}>{cfg.label}</span>
            </div>
          </div>
